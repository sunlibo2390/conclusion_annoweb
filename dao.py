...
class DAO:
    def __init__(self, host: str, username: str, password: str, database: str, authSource: str):
        ...
        self._cols: Dict[str, Collection] = {x: self._db[x] for x in [
            'user', 'task1', 'task2', 'anno', 'arbi', 'hist', 'error'
        ]}  # 数据库表名字典，表名:数据表
    ...
    def reset_user_pwd(self, name, old_pwd, new_pwd):
        ...
        user_query: Optional[Dict[str, Any]] = self._query(
            'user', {'name': name, 'active': True, 'password': old_pwd}, 'one', fields=['uid']
        )
        print(user_query)
        if user_query is None:
            return -1
            ...
            try:
                self._update('user', {'name': name, 'active': True, 'password': old_pwd}, {'password': new_pwd})
    ...
    def is_annotated_by_gid_tid_uid(self, uid, gid, tid):
        result = self._query('anno', {'gid': gid, 'uid': uid, 'tid': tid}, 'one', fields=['anno_list', 'aspect_list'])
    ...
    def is_arbitrated_by_gid_tid_uid(self, uid, gid, tid):
        result = self._query('anno', {'gid': gid, 'uid': {'$ne': uid}, 'tid': tid}, 'all')
        ...
        if all_rejected:
            uid_result = self._query('anno', {'gid': gid, 'uid': uid, 'tid': tid}, 'all')
    ...
    def get_task_list_by_gid_uid(self, uid: int, gid: int, adjudicator: bool = False):
        ...
        tasks: Dict[int, Tuple[str, int]] = {
            ...
            for x in self._query(
                'task%d' % gid, {'tid': {"$in": tid_list}}, 'all', fields=['tid', 'object']
            )
        }
        print('tasks', tasks)
    ...
    def get_anno_by_gid_tid_uid(self, gid, tid, uid):
        print(gid, tid, uid)
        annos = self._query(
            'anno', {'gid': gid, 'tid': tid, 'uid': uid}, 'one',
    ...
    def get_annotated_task_by_gid_tid_uid(self, uid, gid, tid):
        ...
        if arbi_tasks is not None:
            ...
            annotated_tasks = self._query(
                'anno', {'uid': {'$ne': uid}, 'gid': gid, 'tid': tid},
    ...
    def count_annotated_by_uid_gid_tidlist(self, uid, gid, tid_list):
        annotated_tasks = self._query('anno', {'uid': uid, 'gid': gid, 'tid': {'$in': tid_list}}, 'all',
                                      fields=["anno_list", "aspect_list"])
        finished_num = 0
        for task in annotated_tasks:
            '''print(
    ...
    def count_arbitrated_by_uid_gid_tidlist(self, uid, gid, tid_list):
        finished_num = 0
        arbitrated_tasks = self._query('arbi', {'uid': uid, 'gid': gid, 'tid': {'$in': tid_list}}, 'all',
                                       fields=["adjudicated"])
        finished_num = 0
    ...
    def save_task_by_gid_tid_uid(self, uid: int, gid: int, tid: int, anno_list: List[str], aspect_list: List[str],
        ...
        try:
            if anno_state_list is not None:
                # 仲裁环节
                arbi_task = self._query('arbi', {'gid': gid, 'tid': tid, 'uid': uid}, 'one')
                if arbi_task is not None:
                    # 如果之前仲裁过
                    self._remove('arbi', {'gid': gid, 'tid': tid, 'uid': uid})
                self._insert(
                    'arbi', {
                ...
                '''# 遍历该任务下非该用户标记的数据
                annotated_tasks = self._query(
                    'arbi', {'gid': gid, 'tid': tid, 'uid': {'$ne': uid}},
                    'all', fields=['gid', 'tid', 'uid', 'anno'])
                for i, annotated_task in enumerate(annotated_tasks):
                    self._update(
                        'arbi', {'gid': gid, 'tid': tid, 'uid': annotated_task['uid'], 'anno': annotated_task['anno']},
                        {'arbi_uid': uid, 'state': anno_state_list[i]}
                    )
                # 如果是全部拒绝
                # 如果该用户之前有修正，删除
                pre_arbi = self._query('anno', {'gid': gid, 'tid': tid, 'uid': uid, 'arbi_uid': uid}, 'all')
                for j in pre_arbi:
                    self._remove('anno', {'gid': gid, 'tid': tid, 'uid': uid, 'arbi_uid': uid})
                for anno in anno_list:
                    self._insert('anno',
                                 {'gid': gid, 'tid': tid, 'uid': uid, 'anno': anno, 'arbi_uid': uid, 'state': 1})'''
            else:
                # 标注环节
                # 清空该用户对该任务的原有标注
    ...
    def report_task_error(self, uid, gid, tid, error_type, error_info):
        error = self._query('error', {"uid": uid, "gid": gid, "tid": tid}, "one")
        if error is not None:
            self._remove('error', {"uid": uid, "gid": gid, "tid": tid})
        self._insert('error', {"uid": uid, "gid": gid, "tid": tid, "type": error_type, "info": error_info})
